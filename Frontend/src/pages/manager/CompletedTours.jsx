...
const CompletedTours = () => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      setError('');
      const res = await axiosInstance.get('/manager/completed');
      if (res.data.success) {
        setBookings(res.data.data);
      }
    } catch (err) {
      setError('Failed to fetch completed tours');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="page-container">
      <div className="page-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '30px' }}>
        <div>
          ...
          <p>History of successfully concluded trips.</p>
        </div>
      ...
      <div style={{ background: '#fff', padding: '25px', borderRadius: '12px', boxShadow: '0 4px 15px rgba(0,0,0,0.05)' }}>
        ...
        <div style={{ display: 'flex', gap: '15px', marginBottom: '20px', flexWrap: 'wrap' }}>
          ...
          >
            ...
            <option value="createdAt-desc">Booking Date (Newest First)</option>
          </select>
        </div>
            ...
            <tbody>
              {currentItems.length > 0 ? currentItems.map(b => (
                <tr key={b.id}>
                  ...
                  <td>{b.guideId ? `Guide #${b.guideId}` : 'N/A'}</td>
                ...
                <tr>
                  <td colSpan="7" style={{ textAlign: 'center', padding: '20px' }}>No completed tours found.</td>
                </tr>
              )}
            </tbody>
          </table>
