...
const Bookings = () => {
  ...
  const sortedBookings = [...filteredBookings].sort((a, b) => {
    ...
    if (sortField === 'travelDate' || sortField === 'createdAt') {
  ...
  return (
    <div className="page-container">
      <div className="page-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '30px' }}>
        <div>
          <h2>Tour Manager Bookings</h2>
          <p>Manage all bookings and assignments.</p>
        </div>
      ...
      <div style={{ background: '#fff', padding: '25px', borderRadius: '12px', boxShadow: '0 4px 15px rgba(0,0,0,0.05)' }}>
        ...
        <div style={{ display: 'flex', gap: '15px', marginBottom: '20px', flexWrap: 'wrap' }}>
          ...
          >
            <option value="createdAt-desc">Booking Date (Newest)</option>
            <option value="createdAt-asc">Booking Date (Oldest)</option>
            ...
            <tbody>
              {currentItems.length > 0 ? currentItems.map(b => (
                <tr key={b.id}>
                  ...
                  <td><span className="badge badge-primary">{b.bookingStatus.replace('_', ' ')}</span></td>
                  <td>
                    <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                      <Link to={`/admin/bookings/${b.id}`} className="btn-secondary btn-sm" style={{ padding: '5px 10px' }}>View Details</Link>
                      {b.bookingStatus === 'CONFIRMED' && (
                        <Link to={`/manager/assignments`} className="btn-primary btn-sm" style={{ padding: '5px 10px' }}>Assign Logistics</Link>
                      )}
                      <Link to={`/manager/itinerary?bookingId=${b.id}`} className="btn-secondary btn-sm" style={{ padding: '5px 10px' }}>Itinerary</Link>
                    </div>
                  </td>
                </tr>
              )) : (
