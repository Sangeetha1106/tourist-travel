...
const UpcomingTours = () => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      setError('');
      const res = await axiosInstance.get('/manager/upcoming');
      if (res.data.success) {
    ...
    <div className="page-container">
      <div className="page-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '30px' }}>
        <div>
          ...
          <p>Future trips with travel dates greater than today.</p>
        </div>
            ...
            <tbody>
              {currentItems.length > 0 ? currentItems.map(b => (
                <tr key={b.id}>
                  ...
                  <td><span className={`badge ${getStatusBadge(b.bookingStatus)}`}>{b.bookingStatus.replace('_', ' ')}</span></td>
                  <td>
                ...
                <tr>
                  <td colSpan="7" style={{ textAlign: 'center', padding: '20px' }}>No upcoming tours found.</td>
                </tr>
              )}
            </tbody>
          </table>
