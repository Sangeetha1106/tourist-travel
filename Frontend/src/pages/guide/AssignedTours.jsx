...
const AssignedTours = () => {
  ...
  const fetchTours = async () => {
    try {
      setLoading(true);
      const res = await axiosInstance.get('/guide/assigned');
      if (res.data.success) {
  ...
  return (
    <div className="page-container">
      <div className="page-header" style={{ marginBottom: '30px' }}>
        ...
        <p>Tours assigned to you that are ready to be accepted</p>
      </div>
            ...
            <tbody>
              {tours.length > 0 ? tours.map(t => (
                <tr key={t.id}>
                  ...
                  <td><span className="badge badge-warning">{t.bookingStatus}</span></td>
                  <td>
                    <button className="btn-primary btn-sm" style={{ padding: '5px 10px', marginRight: '5px' }} onClick={() => handleAction(t.id, 'accept')}>Accept Tour</button>
                  </td>
                </tr>
              )) : (
                <tr>
